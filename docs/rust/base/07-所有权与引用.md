...
## Rust 如何管通过 "借用检查器" 确保安全性?

变量对其所对应保存的数据有3种权限:

1. 读取(R): 数据可以被复制到另外一个位置
2. 写入(W): 数据可以被修改
3. 拥有(O): 数据可以被移动或释放

这些权限在运行时候并不存在, 只是在编译器内部检查的时候存在, 抽象概念方便理解

默认情况下(如: `let x = 10;`), 变量对其数据具有读取/拥有的权限(RO)

使用 `mut` 关键字, 可以给变量赋予 `W 写入` 权限: `let mut x = 10;`
