...
pub fn first_char_uppercase(input: &str) -> String {
    // 假设传入的是 "rust"
    // 那么 "rust".chars() 会返回 -> Chars(["r", "u", "s", "t"])
    // 在迭代器执行一次 next 后:
    // 1.匹配到的字符是: "r"
    // 2.其他还没有被消费的是: Chars(["u", "s", "t"])
    // 3.所以这个match语句会走到 Some
    //    3.1 匹配到 Some("r") -> "r".to_uppercase() -> "R"
    //    3.2 Chars(["u", "s", "t"]) -> as_str() -> "ust"
    //    3.3 最终 format!("{R}{ust}") 返回新的字符串 "Rust"
...
fn main() {
    let str = first_char_uppercase("rust");
    assert_eq!(str, "Rust");
}
```
...
要自定义迭代器就必须实现 [std::iter::Iterator](https://rustwiki.org/zh-CN/std/iter/trait.Iterator.html) 这个 trait
...
impl<T> Stack<T> {
    ...
    pub fn iter_into(self) -> impl Iterator<Item = T> {
...
fn main() {
    ...
    // 使用 Vec Iterator 遍历, 元素不会从 Stack 中弹出
    let mut stk2 = Stack::new();
    stk2.push(1);
    stk2.push(2);
    stk2.push(3);
    for item in stk2.iter() {
        println!("item: {}", item); // 3,2,1
    }
}
```
